...
export default function MiniDrawer() {
  const handleWebsiteRoutes = useButtonClickHandler("/");
  const theme = useTheme();
  const [open, setOpen] = React.useState(false);
  const [selectedIndex, setSelectedIndex] = React.useState(0); // To track selected item
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Drawer variant="permanent" open={open}>
        ...
        <Divider />
        <List>
          {["User Management", "Courses Management"].map((text, index) => (
            <ListItem key={text} disablePadding sx={{ display: "block" }}>
              ...
              >
                ...
                >
                  {index % 2 === 0 ? <GroupIcon /> : <ImportContactsIcon />}
                </ListItemIcon>
